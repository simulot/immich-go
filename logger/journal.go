...
func (j *Journal) Report() {
	...
	j.Logger.OK("%6d files in the input", j.counts[DiscoveredFile])
	j.Logger.OK("--------------------------------------------------------")
	j.Logger.OK("%6d photos", j.counts[ScannedImage])
	j.Logger.OK("%6d videos", j.counts[ScannedVideo])
	j.Logger.OK("%6d metadata files", j.counts[Metadata])
	j.Logger.OK("%6d files with metadata", j.counts[AssociatedMetadata])
	j.Logger.OK("%6d discarded files", j.counts[Discarded])
	j.Logger.OK("%6d files having a type not supported", j.counts[Unsupported])
	j.Logger.OK("%6d discarded files because in folder failed videos", j.counts[FailedVideo])

	j.Logger.OK("%6d input total (difference %d)", checkFiles, j.counts[DiscoveredFile]-checkFiles)
	j.Logger.OK("--------------------------------------------------------")
	...
	j.Logger.OK("%6d files already on the server", j.counts[ServerDuplicate])
	j.Logger.OK("%6d discarded files because of options", j.counts[NotSelected])
	j.Logger.OK("%6d discarded files because duplicated in the input", j.counts[LocalDuplicate])
	j.Logger.OK("%6d discarded files because server has a better image", j.counts[ServerBetter])
	j.Logger.OK("%6d errors when uploading", j.counts[ServerError])

	j.Logger.OK("%6d handled total (difference %d)", handledFiles, j.counts[ScannedImage]+j.counts[ScannedVideo]-handledFiles)
}
