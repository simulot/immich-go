...
/*
func GetFileMetaData(fsys fs.FS, name string, localTZ *time.Location) (*assets.Metadata, error) {
	f, err := fsys.Open(name)
	if err != nil {
		return nil, err
	}
	defer f.Close()
	return GetFromReader(f, path.Ext(name), localTZ)
}
...
func GetFromReader(rd io.Reader, ext string, localTZ *time.Location) (*assets.Metadata, error) {
	r := newSliceReader(rd)
	...
	case ".heic", ".heif":
		dateTaken, err = readHEIFDateTaken(r, localTZ)
	case ".jpg", ".jpeg", ".dng", ".cr2":
		dateTaken, err = readExifDateTaken(r, localTZ)
	case ".mp4", ".mov":
		dateTaken, err = readMP4DateTaken(r)
	case ".cr3":
		dateTaken, err = readCR3DateTaken(r, localTZ)
	default:
...
func readExifDateTaken(r io.Reader, localTZ *time.Location) (time.Time, error) {
	md, err := getExifFromReader(r, localTZ)
...
func readHEIFDateTaken(r *sliceReader, localTZ *time.Location) (time.Time, error) {
...
func readCR3DateTaken(r *sliceReader, localTZ *time.Location) (time.Time, error) {
	b := make([]byte, searchBufferSize)
